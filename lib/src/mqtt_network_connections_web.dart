...
class WebSocketMqttNetworkConnection implements MqttNetworkConnection {
  ...
  Future<Stream<int>?> connect() async {
    ...
    try {
      await _webSocket?.onOpen.first.timeout(connectionTimeoutDuration);
    } on TimeoutException {
      _webSocket?.close();
      _webSocket = null;
      return null;
    ...
    _onCloseSubscription = _webSocket?.onClose.listen(
      (event) {
        _webSocket = null;
    ...
    return _webSocket?.onMessage.transform(StreamTransformer.fromHandlers(
